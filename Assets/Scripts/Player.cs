...
public class Player : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        velocity.x = Mathf.SmoothDamp(velocity.x, 
            MathUtils.ProcessDeadZone(Input.acceleration.x, MovementDeadZone) * MovementSpeed * Time.deltaTime // дл€ мобильного устройства
            + MathUtils.ProcessDeadZone(Input.GetAxis("Horizontal"), MovementDeadZone) * MovementSpeed * Time.deltaTime // дл€ тестировани€ в редакторе с использованием клавиш
    ...
    {
        // определ€ем положение объекта относительно игрока
        ...
        switch (relPosition)
        {
            ...
            case RelativePosition.Below:
                ...
                { } // платформа ломаетс€ до того, как игрок может прыгнуть
    ...
    /// <summary>
    /// ¬озвращает положение объекта относительно игрока
    /// </summary>
    /// <param name="collision"> оллайдер объекта, положение которого мы провер€ем</param>
    /// <returns></returns>
    private RelativePosition CheckRelativePosition(Collider2D collision)
    {
        ...
        else if (transform.position.x > collision.transform.position.x) // сделал это отдельными ветками дл€ читаемости кода
            return RelativePosition.Aside;
        else
